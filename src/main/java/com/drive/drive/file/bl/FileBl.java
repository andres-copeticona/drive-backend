...
public class FileBl {
  ...
  public FileDto uploadFile(FileDto fileDto, MultipartFile file) {
    try {
      ...
      FolderEntity folder = folderRepository.findById(fileDto.getFolderId())
          .orElseThrow(() -> new RuntimeException("Folder not found"));
      Usuario usuario = usuarioRepository.findById(fileDto.getUserId())
  ...
  public String shareFile(SharedDocumentDto sharedDocumentDto) {
    ...
    notificacionBl.crearNotificacionCompartir(sharedDocumentDto.getEmisorUsuarioId(),
        ...
        "Has compartido el documento '" + file.getTitle() + "' con " + userToShareWith.getNombres() + " "
            + userToShareWith.getPaterno(),
        "compartido");

    notificacionBl.crearNotificacionCompartir(sharedDocumentDto.getReceptorUsuarioId(),
        "Documento Recibido",
        "Has recibido un documento compartido '" + file.getTitle() + "' de " + userEmisor.getNombres() + " "
  ...
  public List<SharedDocumentDto> findAllSharedDocumentsByUserId(Long userId) {
    ...
    return allDocuments.stream().map(sharedDocument -> {
      ...
      String receptorNombre = usuarioRepository.findById(sharedDocument.getReceptor().getUsuarioID()).get()
          .getNombres();
  ...
  public String deleteFile(Long fileId) {
    try {
      // Primero, obtén una lista de todos los usuarios que tienen acceso al archivo
      // para notificarlos
  ...
  public List<FileDto> findRecentFilesByUserId(Long userId) {
    // Encuentra los archivos por usuario, que no estén eliminados y ordenados por
    // fecha de creación
  ...
  public String updateFileCategory(Long fileId, String newCategory) {
    if (!newCategory.equals("Nuevo") && !newCategory.equals("Reemplazado") && !newCategory.equals("Sellado")) {
      throw new IllegalArgumentException(
          "Categoría inválida. Las categorías válidas son: Nuevo, Reemplazado, Sellado.");
    }
  ...
  public Map<String, Long> countCategoriesByUser(Long userId) {
    ...
    Map<String, Long> categoryCounts = files.stream()
        .collect(Collectors.groupingBy(file -> file.getCategoria() == null ? "Sin categoría" : file.getCategoria(),
            Collectors.counting()));
  ...
  public List<SharedDocumentDto> findSharedDocumentsBetweenUsers(Long emisorId, Long receptorId) {
    List<SharedDocumentEntity> sharedDocuments = sharingRepository.findSharedDocumentsBetweenUsers(emisorId,
        receptorId);
    return sharedDocuments.stream().map(document -> {
      ...
      dto.setLinkDocumento(
          getDownloadUrl(document.getDocumento().getEtag(), document.getDocumento().getFolder().getName()));
