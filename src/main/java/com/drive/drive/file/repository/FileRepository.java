package com.drive.drive.file.repository;
...
public interface FileRepository extends JpaRepository<FileEntity, Long> {
    ...
    //find by user id
    List<FileEntity> findByUser_UsuarioIDAndFolder_IdAndDeletedFalse(Long userId, Long folderId);
    //find by access type
    List<FileEntity> findByAccessTypeAndDeletedFalse(String accessType);

    //find by user id en orden descendente
    List<FileEntity> findByUser_UsuarioIDAndDeletedFalseOrderByCreatedDateDesc(Long userId);

    //fin by categoria and user id los que no esten eleiminados
    List<FileEntity> findByCategoriaAndUser_UsuarioIDAndDeletedFalse(String categoria, Long userId);

    //find by user id los que no esten eleiminados
    List<FileEntity> findByUser_UsuarioIDAndDeletedFalse(Long userId);
