...
public class FolderBl {
  ...
  public void deleteFolder(String folderName) {
    try {
      ...
      // Notificar a todos los usuarios que tienen acceso a la carpeta que será
      // eliminada
      ...
      for (FolderEntity folderEntity : folders) {
        ...
        // Actualizar las carpetas que tienen esta carpeta como carpeta padre,
        // estableciendo parentFolder a null
  ...
  public void shareFolder(Long folderId, Long emisorId, Long receptorId) {
    ...
    notificacionBl.crearNotificacionCompartir(receptorId,
        "Carpeta Compartida",
        "Has recibido acceso a la carpeta '" + folder.getName() + "' de " + emisor.getNombres() + " "
            + emisor.getPaterno(),
        "compartida");
  }
  ...
  public List<FolderDto> listSharedFolders(Long userId) {
    ...
    receivedFolders.forEach(
        sharedFolder -> uniqueFolders.put(sharedFolder.getFolder().getId(), convertToDto(sharedFolder.getFolder())));
      ...
      if (!usuario.getUsuarioID().equals(emisorId)) {
        ...
        notificacionBl.crearNotificacionCompartir(usuario.getUsuarioID(),
            ...
            "Tienes acceso a una nueva carpeta: '" + folder.getName() + "' compartida por "
                + usuarioRepository.findById(emisorId).get().getNombres(),
            "compartida");
      }
    }
  }
